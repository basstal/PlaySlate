	...
	else if (bHandleLeftMouseButton)
	{
		...
		else if (MouseDragType == ENovaDragType::DRAG_SETTING_RANGE)
		{
			...
			if (bCanZoomIn || bCanZoomOut)
			{
				...
				TimeSliderArgs.OnViewRangeChanged.ExecuteIfBound(ViewRange, ENovaViewRangeInterpolation::Immediate);
			}
		}
	...
	else if (bHandleLeftMouseButton)
	{
		...
		if (MouseDragType == ENovaDragType::DRAG_NONE)
		{
			if (DistanceDragged > FSlateApplication::Get().GetDragTriggerDistance())
			{
				...
				ActActionSequence::FActActionScrubberMetrics ScrubberMetrics = NovaStaticFunction::GetScrubPixelMetrics(DisplayRate, QualifiedFrameTime, RangeToScreen, 4.0f);
...
void FActActionTimeSliderController::DrawTicks(FSlateWindowElementList& OutDrawElements, const TRange<double>& ViewRange, const ActActionSequence::FActActionScrubRangeToScreen& RangeToScreen, const ActActionSequence::FActActionDrawTickArgs& InArgs) const
{
	...
	if (!NovaStaticFunction::GetGridMetrics(TimeSliderArgs.NumericTypeInterface, InArgs.AllottedGeometry.Size.X, ViewRange.GetLowerBoundValue(), ViewRange.GetUpperBoundValue(), MajorGridStep, MinorDivisions))
	{
		return;
	}
