...
UObject* UActActionBlueprintFactory::FactoryCreateNew(UClass* InClass,
                                                      UObject* InParent,
                                                      FName InName,
                                                      EObjectFlags Flags,
                                                      UObject* Context,
                                                      ...
                                                      FName CallingContext)
{
	...
	if (ParentClass == nullptr || !FKismetEditorUtilities::CanCreateBlueprintOfClass(ParentClass) || !ParentClass->IsChildOf(
		UActActionLogic::StaticClass()))
	{
		FFormatNamedArguments Args;
		Args.Add(TEXT("ClassName"), ParentClass != nullptr ? FText::FromString(ParentClass->GetName()) : LOCTEXT("Null", "(null)"));
		FMessageDialog::Open(EAppMsgType::Ok,
		                     FText::Format(LOCTEXT("CannotCreateActionBlueprint", "Cannot create a action blueprint '{ClassName}'"), Args));
		return nullptr;
	}

	UActActionBlueprint* NewBP = CastChecked<UActActionBlueprint>(FKismetEditorUtilities::CreateBlueprint(
		ParentClass,
		InParent,
		InName,
		BlueprintType,
		UActActionBlueprint::StaticClass(),
	...
	if (NewBP)
	{
		...
		if (ActionBP == nullptr)
		{
			UEdGraph* NewGraph = FBlueprintEditorUtils::CreateNewGraph(NewBP,
			                                                           TEXT("Action Graph"),
			                                                           UActActionBlueprintGraph::StaticClass(),
			                                                           UActActionBlueprintGraphSchema::StaticClass());
