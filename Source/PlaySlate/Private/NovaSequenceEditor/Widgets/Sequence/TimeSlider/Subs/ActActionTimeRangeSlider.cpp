...
FReply SActActionTimeRangeSlider::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (HasMouseCapture())
	{
		...
		if (bHandleDragged)
		{
			...
			TimeSliderController.Pin()->SetViewRange(NewIn, NewOut, ActActionSequence::EActActionViewRangeInterpolation::Immediate);
		}
...
FReply SActActionTimeRangeSlider::OnMouseButtonDoubleClick(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	...
	if (bHandleHovered && TimeSliderController.IsValid())
	{
		...
		{
			TimeSliderController.Pin()->SetViewRange(LastViewRange.GetLowerBoundValue(), LastViewRange.GetUpperBoundValue(), ActActionSequence::EActActionViewRangeInterpolation::Immediate);
		}
	}
	ResetState();
	return FReply::Unhandled();
